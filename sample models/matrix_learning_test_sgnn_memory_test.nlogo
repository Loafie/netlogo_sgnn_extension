;;extensions [ sgnn ]
globals [ vectors correct tried ratio]
breed[ mice mouse ]
breed[ cheese piece-of-cheese ]
mice-own [energy age generation]

to setup
  clear-all
  ;sgnn:set-up 3 6 6 0.005 0.1
  ...
  [
    sprout-cheese 1 [set shape "cheese"]
  ]
  set correct 0
  set tried 0
  set ratio 0
...
to make-vectors
  ...
  foreach range 6 [x ->
    let l []
    foreach range 6 [y -> set l lput random-float 1 l]
    set vectors lput (list l (x + 1)) vectors
  ]
end
...
to go
  ask mice [
    ...
    if any? cheese in-radius 1
    [
      set tried tried + 1
      let vec one-of vectors
      let choice (random 6) + 1;sgnn:make-choice item 0 vec
      if-else (choice = (item 1 vec))
      [
      ...
      ask one-of cheese in-radius 1 [die]
    ]
    ...
    if energy < 0 [;sgnn:clear-brain
      die]
    if age > max-age [;sgnn:clear-brain
      die]
    set age age + 1
  ]
  if ticks mod 5 = 0 [ask one-of patches [sprout-cheese 1 [set shape "cheese"]]]
  if ((ticks mod 10000) = 0) [if (tried > 0) [set ratio correct / tried] set correct 0 set tried 0]
  tick
end
